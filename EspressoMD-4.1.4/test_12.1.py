...
def main(NAME,maxcycle,steps,vsp,vsf,write_velocity,Equil):
    ...
    for i in range(1,maxcycle+1):
        ...
        if min(system.part[:].pos[:,0]) >(BOX_L+compression_l)/2:
            ...
            print(f" position: {min(system.part[:].pos[:,0])}")
...
geometry(D,1).rectangle(BOX_L,BOX_W,BOX_H,compression_w,compression_l)
...
for i in range(1,maxcycle+1):
        ...
        print(i)
        if (i%vsf==0):
            file_name_velocity= "Energy_Eq/flow_" +NAME+ str(i) + ".vtk"
            lbf.print_vtk_velocity( file_name_velocity )


# create vesicle geometry
geometry(D,0).vesicle(d,BOX_L,BOX_W,BOX_H)
...
lbf = espressomd.lb.LBFluidGPU(kT=1.0,
                               ...
                               visc=1.4267,
                               tau=0.01,
                               ext_force_density=[0.02, 0.0, 0.0])
